...
public class ImageWithTextDelegate implements ItemViewDelegate<JSONObject> {
    @Override
    public int getItemViewLayoutId() {
        return R.layout.fragment_a_one_imagewithtext;
    }

    @Override
    public boolean isForViewType(JSONObject item, int position) {
        try {
            return "image_withtext".equals(item.getString("key"));
